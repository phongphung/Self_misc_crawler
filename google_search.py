...
g_key4 = dict(api_key='AIzaSyBgBYhEKlPFAj6dQfaQ9KHZ5GoJVUj5Kqw',\
                     cse_id = '012557082882668088465:o_xz0gin3ws', status=0)

g_key5 = dict(api_key='AIzaSyBKryTarv_6CL_FLOegeCPxP3ulP1cQtgk',\
                     cse_id = '012057603104391785746:zsy1fmbgig4', status=0)

google_keys = [g_key1, g_key2, g_key3, g_key4, g_key5]
google_keys
active_key = 0
...
results = results.loc[:, ['displayLink', 'formattedUrl', 'query', 'snippet', 'title']]

results['check'] = results['formattedUrl'].apply(lambda x: urltools.extract(x).path)
results
results.to_clipboard()
...
for i in df2.index:
    try:
        query = str(df2.loc[i, 'name']) + ' twitter'
        print(query)
        temp = my_search(query, google_keys)
        results = pd.concat([results, temp])
